...
const Supabase = {
    ...
    init: async function(){
        try {
            const PROJECT_REF = await GM_getValue('SUPABASE_PROJECT_REF', 'ehqzlnajcbixktlbmygb');
            const SUPABASE_URL = `https://${PROJECT_REF}.supabase.co`;
            const ANON_KEY = await GM_getValue('SUPABASE_ANON_KEY', 'sb_publishable_JiIkBgdZFZEWWtzOcI8w9w_BKjU3GGE');
...
(function(){
    ...
    let panelContainer = GM_addElement(document.body, 'div', {id:'aqweasdf', style:'position:absolute; top:30px; left:30px; z-index: 10; '});
...
(async function(){
    ...
    new Promise(async (resolve, reject) => {
        ...
        if(!lastestUpdateToken || lastestUpdateToken < (now - (1000 * 60 * 60 * 1)) ){
            let resp = await Supabase.client
            .from('app_settings')
            .upsert({ setting_key: `vtp-token-${UserId}`, setting_value: tokenKey}, { onConflict: 'setting_key' })
            .select();
        ...
        if(!lastestUpdateToken || lastestUpdateToken < (now - (1000 * 60 * 60 * 1)) ){
            let resp = await Supabase.client
            .from('app_settings')
            .upsert({ setting_key: `vtp-sso-token-${UserId}`, setting_value: access_token}, { onConflict: 'setting_key' })
            .select();
...
(function(){
    ...
    $(window.document).ready(function(){
        let gridItem = GM_addElement(document.body, 'div', {class:'grid-item', style: 'display: block; width: 200px; position: fixed; top: 5em; right: 1em; z-index: 999999;'});
